...
def get_documents(current_user):
    ...
    try:
        ...
        # Базовый запрос - для внешних документов нет department_id и author_id
        ...
        if current_user.role == 'admin':
            ...
            # Менеджер видит все документы (внутренние и внешние)
            pass
        else:
            # Обычный сотрудник видит только документы своего отдела
            if table_name == 'external_documents':
                # Для внешних документов только recipient_department_id
                query += " AND d.recipient_department_id = ?"
                params.append(current_user.department_id)
            else:
                # Для внутренних документов - отдел или адресованные ему
        ...
        if author_id:
            # Для внешних документов нет author_id
            if table_name != 'external_documents':
        ...
        for row in rows:
            ...
            # Для внешних документов добавляем sender_name, sender_contact, sender_inn, sender_kpp и received_date
...
def get_document(doc_id, current_user):
    ...
    try:
        ...
        # Для внешних документов нет department_id и author_id, только recipient_department_id
        if table_name == 'external_documents':
            sql = f"""
                SELECT d.*, 
                       dt.name as document_type_name,
...
def create_document(current_user):
    ...
    try:
        ...
        # Применяем правила классификации (жёсткие правила переопределяют значения)
        from backend.services.classification import apply_classification_rules
        metadata = apply_classification_rules(file, metadata)
        
        # Извлекаем значения из обновленных метаданных
        category = metadata['category']
        document_type_id = metadata.get('document_type_id')
        title = metadata['title']
        description = metadata['description']
        department_id = metadata['department_id']
        recipient_department_id = metadata.get('recipient_department_id')
        ...
        # 🔍 ПРОВЕРКА ДУБЛИКАТОВ ПЕРЕД ЗАГРУЗКОЙ (по имени файла в конкретной папке)
        from backend.services.duplicate_detector import duplicate_detector
        duplicate_check = duplicate_detector.check_duplicate_before_upload(file, metadata, file_path)
        
        if duplicate_check.get('is_duplicate'):
            # Создаем уведомление о попытке загрузки дубликата
            from backend.services.notifications import create_duplicate_notification
            ...
            return jsonify({
                ...
                'error': {
                    ...
                    'suggestion': f'Файл с именем "{file.filename}" уже существует в папке {file_path.parent.relative_to(BASE_DIR)}'
                }
            }), 409  # Conflict
        ...
        if category == 'internal':
            ...
            sender_inn = request.form.get('sender_inn', '')
            sender_kpp = request.form.get('sender_kpp', '')
            received_date = request.form.get('received_date') or datetime.now().date().isoformat()
            
            cursor.execute("""
                INSERT INTO external_documents 
                (document_type_id, title, description, file_path, file_name, 
                 file_size, file_extension, sender_name, sender_contact, sender_inn, sender_kpp,
                 recipient_department_id, recipient_user_id, document_date, received_date, expiry_date,
                 work_status, completion_percent)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                document_type_id, title, description, str(relative_path), file.filename,
                file_size, file_ext, sender_name, sender_contact, sender_inn, sender_kpp,
                recipient_department_id, recipient_user_id, document_date, received_date, expiry_date,
                'not_processed', 0
            ))
        ...
        try:
            cursor.execute("""
                ...
                VALUES (?, ?, ?)
            """, (doc_id, category, current_user.id))
            conn.commit()
        except Exception as e:
            print(f"Ошибка логирования загрузки: {e}")
        ...
        # 🔍 ПРОВЕРКА ДУБЛИКАТОВ ПОСЛЕ ЗАГРУЗКИ
        from backend.config import DUPLICATE_CHECK_ENABLED
        duplicates = []
        if DUPLICATE_CHECK_ENABLED:
            duplicates = duplicate_detector.find_duplicates(method='hash')
            
            if duplicates:
                # Создаем уведомление администратору о найденных дубликатах
                from backend.services.notifications import create_duplicate_report_notification
                create_duplicate_report_notification(duplicates)
        
        return jsonify({
            'success': True,
            'document_id': doc_id,
            'message': 'Документ успешно загружен',
            'duplicates_found': len(duplicates) if DUPLICATE_CHECK_ENABLED and duplicates else 0
        }), 201
